...
const AyahCard = React.memo(({ ayah, index, currentlyPlayingAyahIndex, isSelected, toggleAyahSelection, handleAyahClick, currentLanguages, translationNames, ayahRefs, highlightColor, arabicFontSize, translationFontSize }) => {
    ...
    >
      <CardBody>
        ...
        {currentLanguages.map(lang => {
          ...
          return translation ? (
            <Box key={lang} mt={3}>
              <Text fontSize="sm" color={grayTextColor} mb={1}> {/* Use theme-aware text color */}
                <strong>{translationNames[lang] || lang}:</strong>
              </Text>
              <Text fontSize={`${translationFontSize}px`}>
                {translation.text}
              </Text>
            </Box>
          ) : null;
            ...
            {showTafsir && (
              <Box>
                {validTafsirs.map((tafsir, idx) => (
                  <Box key={idx} mt={3} p={3} bg={purpleBg} borderRadius="md" borderWidth="1px" borderColor={purpleBorder}>
...
const QuranReaderPage = ({ colorMode, toggleColorMode }) => { // Renamed component
  ...
  const borderColor = useColorModeValue('gray.200', 'gray.600'); // Added for general borders
  const inputBg = useColorModeValue('white', 'dark.inputBg'); // Use theme-defined dark.inputBg
  const inputBorder = useColorModeValue('gray.200', 'dark.inputBorder'); // Use theme-defined dark.inputBorder
  const optionBg = useColorModeValue('white', 'dark.optionBg'); // Use theme-defined dark.optionBg
  const optionColor = useColorModeValue('gray.800', 'dark.optionColor'); // Use theme-defined dark.optionColor
  ...
  const handleSurahChange = useCallback(async (event) => {
    ...
    if (selectedSurah) {
      ...
      try {
        ...
        // Re-added setAyahRange here to initialize it with the correct number of ayahs
        // This is the key change from the previous working version for bulk mode
            ...
            <Tooltip label="Bulk Selection Mode">
              <IconButton
                ...
                onClick={() => {
                  ...
                  setSelectedAyahs([]);
                  stopAudio();
                  // This was the logic from the previous working version for bulk mode
                  if (surahInfo) {
                    setAyahRange([1, surahInfo.numberOfAyahs]);
                  } else {
                    setAyahRange([1, 1]); // Default if no surah is loaded
                  }
                }}
                  ...
                  >
                    {surahs.map((surah) => (
                      <option
                        value={surah.number}
                        key={surah.number}
                        // Explicitly apply style for options to ensure dark mode visibility
        ...
        <DrawerContent bg={modalBg}>
          ...
          <DrawerBody>
            <Stack spacing={4}>
              ...
              <FormControl>
                <FormLabel htmlFor="highlight-color-picker" color={modalBodyTextColor}>Playing Ayah Highlight Color</FormLabel>
                <Flex alignItems="center">
                  <Input
                    ...
                    bg={inputBg}
                  />
                  <Text ml={3} color={modalBodyTextColor}>{highlightColor}</Text>
                </Flex>
              </FormControl>
...
export default QuranReaderPage; // Export with the new name