...
const AyahCard = React.memo(({ ayah, index, currentlyPlayingAyahIndex, isSelected, toggleAyahSelection, handleAyahClick, currentLanguages, translationNames, ayahRefs, highlightColor, arabicFontSize, translationFontSize, grayTextColor, arabicTextColor, cachedAyahAudio, arabicFontFamily }) => {
...
const QuranReaderPage = ({ colorMode, toggleColorMode }) => {
  ...
  const handleSurahChange = useCallback(async (surahNum, script = arabicScript) => {
    ...
    try {
        ...
        if (linkParams) {
          console.log(`[SHARED LINK LOG] Detected shared link. Attempting to load memorization set for Surah ${surahNum}.`);
        }
            ...
            {selectedSurahData
              ...
              return (
                <AyahCard
                  ...
                  // NEW: Pass the arabicScript state as the arabicFontFamily prop
                  arabicFontFamily={arabicScript}
                />
              );
            })}
            ...
            <Stack spacing={4}>
              ...
              <FormControl>
                ...
                <Select
                  ...
                  color={optionColor}
                >
                  <option value="KFGQPC HAFS Uthmanic Script Regular">Uthmanic (Hafs)</option>
                  <option value="KFGQPC Uthman Taha Naskh">Uthmanic (Taha Naskh)</option>
                  {/* Removed old options */}
                </Select>
              </FormControl>
